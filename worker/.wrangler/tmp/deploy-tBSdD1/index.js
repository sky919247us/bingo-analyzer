  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/kv/backfill" && request.method === "POST") {
        ...
        if (body.today_draws)
          await env.BINGO_KV.put("today_draws", JSON.stringify(body.today_draws));
        if (body.latest_draw)
          await env.BINGO_KV.put("latest_draw", JSON.stringify(body.latest_draw));
        if (body.last_updated)
          await env.BINGO_KV.put("last_updated", body.last_updated);
        if (body.today_date)
          await env.BINGO_KV.put("today_date", body.today_date);
...
async function syncDrawsToKV(env) {
  try {
    ...
    const adjustedNow = new Date(now.getTime() - 4 * 60 * 60 * 1e3);
...
__name(syncDrawsToKV, "syncDrawsToKV");
export {
